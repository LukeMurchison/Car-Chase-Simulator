...
def main():
  ...
  cop_type = ["City Police", "State Troopers", "Unknown Federal Agency", "Sheriff's Department"]
  cop.type = random.choice(cop_type)
  events = ["4-way", "stop light", "stop sign" "Sharp left", "Sharp right", "Spike Strip", "pothole", "road block", "road work"]
  player.speed = 35
  cop.speed = 35
  wrecked = False
  can_turn = True
  ...
  while game(player, cop):
    ...
    #reverse
    if reverse == True:
        print("You're going in reverse now! if you don't fowrd soon you'll be caught. ")
        action_num -= 1
        reverse_num += 1
  ...
  # event 4-way
    if randevents == "4-way":
        action = input("""There's a 4-way ahead. Which way do you want to go?
[Straight] - [Left] - [Right] - [quit]""").upper()
        if action == "STRAIGHT":
  ...
  # event stop light
    if randevents == "stop light" or randevents == "stop sign":
        print("""There's a stop light down the road, you can't tell the color yet. What do you do? """)
